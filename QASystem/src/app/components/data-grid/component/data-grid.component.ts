...
import { DataGridService } from './service/data-grid.service'
@Component({
...
export class MatPaginatorDataTableComponent {
  @Input() pageSizeOptions = [1, 5, 10, 25, 100];
...
export class MatColumnTableComponent {
  ...
  @Input() sort = false;
  @Input() filter = true;
  @Input() class: string = 'warning';
  @Input() defaultFilter: TFilter = 'equals';
...
export class DataGridComponent
  ...
  public reloadSubject: Subject<unknown>;
      ...
      merge(
        ...
        .pipe(
          ...
          map((data: unknown) => {
            ...
            if ((data as IPaginateOutDTO<unknown>).items.length) {
              return (data as IPaginateOutDTO<unknown>).items;
            } else {
              for (let key in (data as IPaginateOutDTO<unknown>).items) {
                auxData.push((data as IPaginateOutDTO<unknown>).items[key]);
              }
              return auxData;
            }
