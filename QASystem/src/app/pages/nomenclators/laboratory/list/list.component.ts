...
import { ILaboratory } from '../../../../interfaces/entity-interfaces';
...
export class ListComponent implements OnInit {
  ...
  constructor(
    public service: LaboratoryService,
  ...
  update(id: string) {
    this.router.navigate(['pages/nomenclators/laboratory/edit', id]);
  }
    ...
    dialogRef.afterClosed().subscribe((result: string) => {
      if (result == 'accept') {
        ...
        if (this.tabla.selection.selected.length != 0) {
          this.service
            .deleteMany(ids, 'deleteManyLaboratory')
            .pipe(
              catchError((err) => {
