...
import { DefencyTypeService } from 'src/app/services/nomenclators/defency-type.service';
...
import { IDeficiency } from '../../../../interfaces/entity-interfaces';
...
export class ListComponent implements OnInit {
  ...
  constructor(
    public service: DefencyTypeService,
  ...
  update(id: string) {
    this.router.navigate(['pages/nomenclators/deficiency/edit', id]);
  }
    ...
    dialogRef.afterClosed().subscribe((result: string) => {
      if (result == 'accept') {
        ...
        if (this.tabla.selection.selected.length != 0) {
          this.service
            .deleteMany(ids, 'deleteManyDeficiency')
            .pipe(
              catchError((err) => {
