...
import { INotification } from './../../interfaces/entity-interfaces';
...
export class NotificationsService {
  ...
  findAllPagination(
    ...
    const page: IPaginateDTO = {
      ...
      select: [JSON.parse(sessionStorage.getItem('currentUser')!).username],
    };
    return this.httpClient.post<any>( this.enviroment.config.url + 'storeNotification',
  ...
  get(user: any, service: string): Observable<any> {
    return this.httpClient.post<any>( this.enviroment.config.url + service, { user }, httpOptions).pipe(map((resp: any) => {
      return resp;
    }));
  }
