...
export class KillFloorTempService {
  url = conf.url;
  ...
  findAllPagination(
    ...
    return this.httpClient.post<any>(this.enviroment.config.url + 'storeKillFlooTemp',
      page).pipe(map((data): unknown => {
        return (
          data.data as {
            sop_log: IPreOperation[];
          }
        ).sop_log;
      }))
  }
  ...
  create(data: any, service: string): Observable<any> {
   console.warn("LLEGA AL SERVICIO",data)
    return this.httpClient.post<any>(this.enviroment.config.url + service, {
      ...
      priot_tostar_up:data.time,
      relapse_actions_id:data.relapse_actions_id
